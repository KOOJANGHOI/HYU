...
public class ElephantAI extends AbstractAI implements AI{
	 ...
	 * {@inheritDoc}
	 * 
	 * {@link Elephant} do action in the {@link world}.
	 * 
	 * Especially Elephant do move, eat, breed.
	 * 
	 ...
	 * {@link Grass} in adjacent direction, return {@link EatCommand}
	 * toward that direction. If there are no empty adjacent {@link Grass} then return null.
	 * 
	 * In case of breed, It choose random adjacent direction and if there are no
	 * object(empty) in that direction and Elephant has sufficient energy, return
	 * {@link BreedCommand} toward that direction If there are no empty adjacent
	 * {@link Location} or Elephant has insufficient energy, then return null.
	 * 
	...
	public Command act(World world, Actor actor) {
		...
		int choose = generator.nextInt(20);
		if (choose < 4) { // move
			for (int i = 0; i < 4; i++) {
				dir = Util.randomDir();
