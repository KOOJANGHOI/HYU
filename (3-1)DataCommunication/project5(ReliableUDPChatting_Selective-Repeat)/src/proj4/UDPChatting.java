...
public class UDPChatting {
	...
	public static DatagramPacket resendpacket;
	...
	public static String[] msgarr = new String[1000];
	public static int[] msglenarr = new int[1000];
	...
	public static byte Sw = 4, Sf = 0, Sn = 0, Rn = 1; // N(S) , N(R) // 없던 코드
														// 원래 NR =1
			...
			while (sRun) {
				...
				if (check) {
					if (Signaling.ACKNOTIFY) {
						// RcvThread가 ACK를 수신했을때 , Timeout을 초기화
						ticks.TimeoutReset((byte) (signal.ACKNO-1));
					}
					else if (!Signaling.ACKNOTIFY) {
						// RcvThread가 ACK를 수신하지 못하였을때 , 패킷 재전송 후 Timeout을 재설정
						System.out.println("\nRetransmission[BecauseOfNoAck][No."+signal.ACKNO+"][Message:" + msgarr[signal.ACKNO] + "]");
						int datalength3 = msglenarr[signal.ACKNO];
						int totallength3 = datalength3 + 22;
						byte[] lenpdu3 = new byte[2];
						byte[] temp4 = ByteBuffer.allocate(4).putInt(totallength3).array();
						lenpdu3[0] = temp4[2];
						lenpdu3[1] = temp4[3]; 
						byte[] control3 = new byte[2];
						control3[0] = (byte) (0x7F & signal.ACKNO);
						control3[1] = (byte) (0x7F & Rn);
						buffer = new Framing(dstaddr, srcaddr, lenpdu3, dsap, ssap, control3, msgarr[signal.ACKNO].getBytes())
								.framing();
						resendpacket = new DatagramPacket(buffer, buffer.length,
								remoteaddr, remoteport);
						socket.send(resendpacket);
						if(signal.ACKNO == (byte)0)
							continue;
						ticks.TimeoutSet((byte) (signal.ACKNO), 3000, signal);
						signal.waitingACK();
					}
					
					if (Signaling.NAKNOTIFY) {
						// RcvThread가 NAK를 수신을을때 , 해당 index의 frame 재전송 후 Timeout을 재설정
						System.out.println("\nRetransmission[BecauseOfNAK][No."+signal.NAKNO+"][Message:" + msgarr[signal.NAKNO] + "]");
						int datalength2 = msglenarr[signal.NAKNO];
						int totallength2 = datalength2 + 22;
						byte[] lenpdu2 = new byte[2];
						byte[] temp3 = ByteBuffer.allocate(4).putInt(totallength2).array();
						lenpdu2[0] = temp3[2];
						lenpdu2[1] = temp3[3]; 
						byte[] control2 = new byte[2];
						control2[0] = (byte) (0x7F & signal.NAKNO);
						control2[1] = (byte) (0x7F & Rn);
						buffer = new Framing(dstaddr, srcaddr, lenpdu2, dsap, ssap, control2, msgarr[signal.NAKNO].getBytes())
								.framing();
						resendpacket = new DatagramPacket(buffer, buffer.length,
								remoteaddr, remoteport);
						socket.send(resendpacket);
						ticks.TimeoutSet((byte) (signal.NAKNO), 3000, signal);
						signal.waitingNAK();
					}
				}
				check = true;
				...
				if (datalength == 4 && sendMessage.equalsIgnoreCase("DISC")) {
					...
					// message를 Framing
					buffer = new Framing(dstaddr, srcaddr, lenpdu, dsap, ssap, control, sendMessage.getBytes())
							.framing();
					...
					if ((remoteaddr != null)) {
						// 패킷 생성
						packet = new DatagramPacket(buffer, buffer.length, remoteaddr, remoteport);
						msgarr[Sn] = sendMessage;
						msglenarr[Sn] = sendMessage.length();
						
						/*
						//여기 주석을 풀면 Selective-Repeat ARQ가 제대로 동작하는지 알수있다.
						if((test == 0 && Sn == (byte)2)) {
							Sn = (byte) ((Sn + 1) % 128);
							test = 1;
						}else {
						...
						ticks.TimeoutSet(Sn, 3000, signal);
						Sn = (byte) ((Sn + 1) % 128);
