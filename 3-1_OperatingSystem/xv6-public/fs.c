...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NINDIRECT*NINDIRECT) {
    ...
    if((addr = ip->addrs[NDIRECT+1]) == 0)   // at index 12
        ip->addrs[NDIRECT+1] = addr = balloc(ip->dev);
    bp = bread(ip->dev , addr);
    a = (uint*)bp->data;

    // if first-address array is empty
    if((addr = a[(bn / NINDIRECT)]) == 0) {
        // balloc and insert address , log_write
    ...
    // if second-address array is empty
    if((addr = a[(bn % NINDIRECT)]) == 0) {
        // balloc and insert address , log_write
        a[(bn % NINDIRECT)] = addr = balloc(ip->dev);
        log_write(bp);
  ...
  if(ip->addrs[NDIRECT+1]) {
      ...
      for(i = 0 ; i < NINDIRECT ; i++) {
          if(a[i]) {
              ...
              for(j = 0 ; j < NINDIRECT ; j++) {
                  if(a2[j]) {
                      // free second-address array
                      bfree(ip->dev , a2[j]);
                  }
              }
              brelse(bp2);
              // free first-address array
              bfree(ip->dev , a[i]);
              a[i] = 0;
          }
      }
